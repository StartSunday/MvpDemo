...
/**
 * 重写系统 FragmentTabHost ：更改他的切换其他fragment时，销毁当前fragment的机制，避免重复加载的机制
 * 修改doTabChanged ： ft.attach(newTab.fragment)->ft.show(newTab.fragment);
 */
public class MyFragmentTabHost extends TabHost
    ...
    @Nullable
    private FragmentTransaction doTabChanged(@Nullable String tabId,
    ...
    @Nullable
    public TabInfo getTabInfoForTag(String tabId) {
        for (int i = 0, count = mTabs.size(); i < count; i++) {
            final TabInfo tab = mTabs.get(i);
            if (tab.tag.equals(tabId)) {
                return tab;
